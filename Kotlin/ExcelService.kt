...
import com.belasoft.natabase_alpha.utils.InputSanitizer
...
object ExcelService {
    ...
    private const val FILE_NAME_PREFIX = "mapa_producao_"

    private fun getFileName(context: Context): String {
        val account = GoogleSignIn.getLastSignedInAccount(context)
        val userId = account?.id ?: "anonymous"
        return "${FILE_NAME_PREFIX}${userId}.xlsx"
    }

    fun getCurrentExcelFile(context: Context): File {
    ...
    private fun sanitizeProductData(produtos: List<Array<String>>): List<Array<String>> {
        return produtos.map { produto ->
            arrayOf(
                InputSanitizer.sanitizeProductInput(produto[0]), // Categoria
                InputSanitizer.sanitizeProductInput(produto[1]), // Nome
                InputSanitizer.sanitizeProductInput(produto[2])  // Validade
            )
        }
    }

    fun salvarProducaoSegura(context: Context, mapa: MapaProducao, producaoIndex: Int): Boolean {
        // Validar todas as quantidades antes de salvar
        val itensInvalidos = mapa.itens.filter { item ->
            !InputSanitizer.isValidQuantity(item.quantidade) ||
                    item.producoes.any { !InputSanitizer.isValidQuantity(it.quantidade) } ||
                    !InputSanitizer.isValidQuantity(item.perdas) ||
                    !InputSanitizer.isValidQuantity(item.sobras)
        }

        if (itensInvalidos.isNotEmpty()) {
            throw SecurityException("Dados de produção contêm valores inválidos")
        }

        return salvarProducao(context, mapa, producaoIndex)
    }
    ...
    fun salvarProducaoIndividual(context: Context, mapa: MapaProducao, produtoNome: String): Boolean {
        val fileName = getFileName(context)
        val file = File(context.getExternalFilesDir(null), fileName)
        FileInputStream(file).use { fis ->
            val workbook = XSSFWorkbook(fis)
            ...
            val cellData = sheet.getRow(0).getCell(13)
            if (cellData == null || cellData.toString().isBlank()) {
                val hoje = SimpleDateFormat("dd/MM/yyyy", Locale.getDefault()).format(Date())
                val novaCellData = sheet.getRow(0).getCell(13) ?: sheet.getRow(0).createCell(13)
                novaCellData.setCellValue(hoje)
            }

            val cellDia = sheet.getRow(1).getCell(13)
            if (cellDia == null || cellDia.toString().isBlank()) {
                val diaSemana = SimpleDateFormat("EEEE", Locale("pt", "PT")).format(Date())
                val novaCellDia = sheet.getRow(1).getCell(13) ?: sheet.getRow(1).createCell(13)
                novaCellDia.setCellValue(diaSemana.capitalize(Locale.getDefault()))
            }
            ...
            while (rowIndex <= sheet.lastRowNum) {
                ...
                val currentProdutoNome = row.getCell(1)?.toString() ?: ""
                if (currentProdutoNome == produtoNome) {
                    for (i in 1..MAX_PRODUCAO) {
                        val colunaQtTest = 3 + (i - 1) * 2
                        val cellQt = row.getCell(colunaQtTest)
                        if (cellQt == null || cellQt.toString().isBlank()) {
                            producaoIndex = i
                            colunaQt = colunaQtTest
                            colunaHr = colunaQt + 1
                            break
                        }
                    }
            ...
            if (producaoIndex > MAX_PRODUCAO) {
                producaoIndex = MAX_PRODUCAO
                colunaQt = 3 + (MAX_PRODUCAO - 1) * 2
                colunaHr = colunaQt + 1
            }
            ...
            while (rowIndex <= sheet.lastRowNum) {
                ...
                if (currentProdutoNome == produtoNome) {
                    ...
                    if (item != null) {
                        val quantidade = item.producoes.firstOrNull()?.quantidade ?: 0
        ...
        FileInputStream(file).use { fis ->
            ...
            while (rowIndex <= sheet.lastRowNum) {
                ...
                if (produtoNome.isNotBlank()) {
                    val item = mapa.itens.find { it.produto == produtoNome }
                    if (item != null) {
                        if (producaoIndex <= item.producoes.size) {
                            val producao = item.producoes[producaoIndex - 1]
