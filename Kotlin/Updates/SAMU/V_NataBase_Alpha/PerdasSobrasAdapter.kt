...
class PerdasSobrasAdapter(
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.btnPerdas.setOnClickListener {
            val bottomSheet = CalculadoraBottomSheet.newInstance(item.produto, "Perdas", 0)
            bottomSheet.setListener(object : CalculadoraBottomSheet.CalculadoraListener {
                override fun onValorConfirmado(produtoNome: String, tipoOperacao: String, valor: Int, producaoIndex: Int) {
                    if (validarPerdasSobras(produtoNome, tipoOperacao, valor)) {
                        atualizarItem(produtoNome, tipoOperacao, valor)
                    } else {
                        Toast.makeText(holder.itemView.context, "Erro: Perdas + Sobras não podem exceder a produção total!", Toast.LENGTH_LONG).show()
                    }
                }
            })
            bottomSheet.show((holder.itemView.context as FragmentActivity).supportFragmentManager, "calculadora")
        }

        holder.btnSobras.setOnClickListener {
            val bottomSheet = CalculadoraBottomSheet.newInstance(item.produto, "Sobras", 0)
            bottomSheet.setListener(object : CalculadoraBottomSheet.CalculadoraListener {
                override fun onValorConfirmado(produtoNome: String, tipoOperacao: String, valor: Int, producaoIndex: Int) {
                    if (validarPerdasSobras(produtoNome, tipoOperacao, valor)) {
                        atualizarItem(produtoNome, tipoOperacao, valor)
